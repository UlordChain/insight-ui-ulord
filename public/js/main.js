...
var defaultCurrency = localStorage.getItem('insight-currency') || 'UT';
...
angular.module('insight.blocks').controller('BlocksController',
  function($scope, $rootScope, $routeParams, $location, Global, Block, Blocks, BlockByHeight, Transaction) {
  $scope.global = Global;
  $scope.loading = false;
      ...
      Transaction.get({
        ...
        //console.log(res);
     	$rootScope.titleDetail = block.height;
      	$rootScope.flashMessage = null;
      	$scope.loading = false;
        block.hash = $routeParams.blockHash;
      	block.reward = res.valueOut;
      	$scope.block = block;
      })
    }, function(e) {
      if (e.status === 400) {
  ...
  function($scope, $rootScope, Currency) {
    ...
    $rootScope.currency.getConvertion = function(value) {
      ...
      if (!isNaN(value) && typeof value !== 'undefined' && value !== null) {
        ...
        } else if (this.symbol === 'mUT') {
          this.factor = 1000;
          response = _roundFloat((value * this.factor), 5);
        } else if (this.symbol === 'uUT') {
          this.factor = 1000000;
          response = _roundFloat((value * this.factor), 2);
        } else {
          this.factor = 1;
    ...
    $scope.setCurrency = function(currency) {
      ...
      } else if (currency === 'mUT') {
        $rootScope.currency.factor = 1000;
      } else if (currency === 'uUT') {
...
angular.module('insight.system').controller('ScannerController',
  function($scope, $rootScope, $modalInstance, Global) {
    ...
    qrcode.callback = function(data) {
      ...
      var str = (data.indexOf('ulord:') === 0) ? data.substring(8) : data;
...
angular.module('insight').config(function($routeProvider) {
  $routeProvider.
    ...
    when('/blocks', {
      ...
      title: 'Ulord Blocks solved Today'
    }).
    when('/blocks-date/:blockDate/:startTimestamp?', {
      templateUrl: 'views/block_list.html',
      title: 'Ulord Blocks solved '
    }).
